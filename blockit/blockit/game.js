...
class gamePlay extends Phaser.Scene {
  ...
  create() {
    ...
    this.theBall = this.physics.add.image(
      game.config.width / 2,
    ...
    this.physics.add.collider(
      ...
      function(ball, wall) {
        ...
        if (wall.x == this.lowerWall.x && wall.y == this.lowerWall.y) {
          ...
          let ballVelocity = this.physics.velocityFromAngle(
  ...
  update() {
    if (
      (this.theBall.y > game.config.height || this.theBall.y < 0) &&
      !this.gameOver
    ) {
      this.gameOver = true;
